...
import { 
  ...
  Image as ImageIcon, Layers, Play, Check, HelpCircle, Upload
} from "lucide-react";
...
export default function GameManager({ user }: GameManagerProps) {
  ...
  // Helper to compress image data url via HTML5 Canvas
  const compressImage = (
    dataUrl: string,
    maxWidth: number,
    maxHeight: number,
  ...
  const handleImageFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type.toLowerCase())) {
      setFormError("Vui lòng chọn tệp hình ảnh định dạng: JPG, JPEG, PNG hoặc WEBP.");
      return;
    }

    if (file.size > 10 * 1024 * 1024) {
      setFormError("Dung lượng tệp ảnh không được vượt quá 10MB.");
      return;
    }
    ...
    reader.onload = (event) => {
      ...
      compressImage(rawDataUrl, 800, 800, 0.85, (compressedDataUrl) => {
              ...
              <div className="space-y-2">
                ...
                {imageUrl ? (
                  <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 space-y-3">
                    <div className="relative h-40 w-full rounded-xl overflow-hidden bg-slate-900 border border-slate-200 group">
                      <img
                        src={imageUrl}
                      ...
                      />
                      <div className="absolute top-2.5 right-2.5 bg-slate-950/80 backdrop-blur-md text-amber-300 border border-amber-400/30 text-[10px] font-black px-2.5 py-1 rounded-lg">
                        ✨ Ảnh xem trước
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs px-3.5 py-2 rounded-xl transition-all cursor-pointer shadow-xs active:scale-95"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold text-xs px-3.5 py-2 rounded-xl border border-rose-200 transition-all cursor-pointer active:scale-95"
                      >
                  ...
                  <div
                    onClick={() => fileInputRef.current?.click()}
                    className="border-2 border-dashed border-slate-300 hover:border-indigo-500 bg-slate-50 hover:bg-indigo-50/40 rounded-2xl p-5 text-center cursor-pointer transition-all space-y-2 group"
                  >
                    <div className="w-12 h-12 bg-white border border-slate-200 group-hover:border-indigo-300 rounded-2xl flex items-center justify-center mx-auto text-indigo-600 shadow-xs group-hover:scale-105 transition-transform">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                        Chỉ chấp nhận các tệp định dạng: <strong>JPG, JPEG, PNG, WEBP</strong>
                      </p>
                    </div>
                  </div>
                )}
